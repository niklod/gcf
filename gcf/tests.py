...
class ViewsTest(TestCase):
    def test_games_detail_view(self):
        ...
        response = self.client.get(
            reverse("gcf:game_detail", kwargs={"game_slug": test_game.slug})
        )
    ...
    def test_player_detail_view_without_config(self):
        ...
        response = self.client.get(
            reverse(
                "gcf:player_detail",
                kwargs={"player_slug": test_player.slug, "game_slug": test_game.slug},
            )
        )
    ...
    def test_player_detail_view_with_twi_configs(self):
        ...
        response = self.client.get(
            reverse(
                "gcf:player_detail",
                kwargs={"player_slug": test_player.slug, "game_slug": test_game_1.slug},
            )
        )
        ...
        response = self.client.get(
            reverse(
                "gcf:player_detail",
                kwargs={"player_slug": test_player.slug, "game_slug": test_game_2.slug},
            )
        )
...
class ModelsCreateTest(TestCase):
    def test_update_player(self):
        test_player = Player(
            name="Test Player", age=123, description="Test Description"
        )
        test_player.save()
